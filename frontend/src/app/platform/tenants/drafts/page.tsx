...
import { TenantDetailShell } from '@/components/tenants/TenantDetailShell';
...
import type { TenantOverviewDraft } from '@/components/tenants/TenantInlineOverviewEditor';
import { tenantCreateHref, tenantListHref } from '@/lib/tenantRoutes';
...
function continueHref(draftId: string) {
    return `${tenantCreateHref()}?draftId=${encodeURIComponent(draftId)}`;
}

export default function TenantDraftsPage() {
    const router = useRouter();
    const [refresh, setRefresh] = React.useState(0);
    const drafts = React.useMemo(() => draftService.getDrafts<TenantOverviewDraft>('tenant'), [refresh]);

    const onDelete = (draft: DraftRecord) => {
        const data = draft.data as TenantOverviewDraft | undefined;
        const label = data?.name?.trim() || 'Untitled tenant';
    ...
    const rows = React.useMemo(() => {
        return drafts.map((d) => {
            const data = d.data as TenantOverviewDraft | undefined;
            const name = data?.name?.trim() || 'Untitled tenant';
    ...
    return (
        <TenantDetailShell breadcrumbItems={[{ label: 'Tenants', href: tenantListHref() }, { label: 'Drafts' }]}>
                ...
                <div className="mt-4 space-y-4">
                            ...
                            columns={[
                                {
                                    ...
                                    render: (row: (typeof rows)[number]) => {
                                        const initial = (row.name.trim().charAt(0) || 'T').toUpperCase();
                                        return (
                    ...
                    {drafts.length === 0 ? (
                        <div className="rounded-xl border border-slate-200 bg-white p-8 text-center shadow-sm">
                            ...
                            <p className="mt-1 text-sm text-slate-600">Start creating a tenant and drafts will appear here when you save a draft.</p>
