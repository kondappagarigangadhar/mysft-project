...
import { InvoiceDetailShell } from '@/components/invoices/InvoiceDetailShell';
...
export default function InvoiceDraftsPage() {
    ...
    const drafts = React.useMemo(() => draftService.getDrafts<InvoiceDraftData>('invoice'), [refresh]);

    const onDelete = (draft: DraftRecord) => {
        const data = draft.data as InvoiceDraftData | undefined;
        const label = data?.invoiceNumber?.trim() || data?.invoiceId?.trim() || 'Untitled Invoice';
    ...
    const rows = React.useMemo(() => {
        return drafts.map((d) => {
            const data = d.data as InvoiceDraftData | undefined;
            const number = data?.invoiceNumber?.trim() || data?.invoiceId?.trim() || 'Untitled Invoice';
            const project = data?.linkedProject?.trim() || '—';
            const party = data?.partyName?.trim() || '—';
            const due = data?.dueDate?.trim() || '—';
            return { draftId: d.draftId, draft: d, number, project, party, due, updatedAt: d.updatedAt };
        });
    }, [drafts]);
    ...
    return (
        <InvoiceDetailShell
            breadcrumbItems={[
                ...
                { label: 'Invoice & Payments', href: '/company-admin/invoices' },
                { label: 'Drafts' },
            ]}
        >
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                        <Table
                            ...
                            onRowClick={(row: (typeof rows)[number]) => {
                                router.push(`/company-admin/invoices/view/new?tab=overview&draftId=${encodeURIComponent(row.draftId)}`);
                            }}
                            columns={[
                                {
                                    key: 'number',
                                    ...
                                    render: (row: (typeof rows)[number]) => {
                                        const initial = (row.number.trim().charAt(0) || 'I').toUpperCase();
                                        return (
                                            <div className="flex items-center gap-3 min-w-0">
                                                ...
                                                <Link
                                                    href={`/company-admin/invoices/view/new?tab=overview&draftId=${encodeURIComponent(row.draftId)}`}
                                ...
                                },
                                {
                                    ...
                                    render: (row: (typeof rows)[number]) => (
                                        <span className="text-sm font-medium text-slate-700">{row.party}</span>
                                    ),
                                },
                                {
                                    key: 'due',
                                    ...
                                    className: 'hidden lg:table-cell',
                                    render: (row: (typeof rows)[number]) => (
                                        <span className="text-sm font-medium text-slate-700">{row.due}</span>
                                    ),
                                },
                                {
                                    key: 'updatedAt',
                    ...
                    {drafts.length === 0 ? (
                        <div className="rounded-xl border border-slate-200 bg-white p-8 text-center shadow-sm">
                            ...
                            <p className="mt-1 text-sm text-slate-600">
                                Start creating an invoice and drafts will appear here automatically.
                            </p>
