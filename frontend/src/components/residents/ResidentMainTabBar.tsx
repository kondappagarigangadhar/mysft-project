...
import {
    RESIDENT_WORKSPACE_TAB_ORDER,
    type ResidentWorkspaceTabId,
} from '@/components/residents/residentDetailTabIds';
import { LuCalendarClock, LuClipboardList, LuHistory, LuLayoutGrid, LuMegaphone, LuQrCode, LuSignature } from 'react-icons/lu';
import { CTA_FOCUS_VISIBLE_RING } from '@/lib/theme/ctaThemeClasses';
import { cn } from '@/lib/utils';

const TAB_META: Record<ResidentWorkspaceTabId, { label: string; icon: typeof LuLayoutGrid }> = {
    overview: { label: 'Overview', icon: LuLayoutGrid },
    notices: { label: 'Notices & Announcements', icon: LuMegaphone },
    visitors: { label: 'Visitors', icon: LuQrCode },
    amenities: { label: 'Amenities', icon: LuCalendarClock },
    records: { label: 'Resident Records', icon: LuClipboardList },
    lease: { label: 'Rental Lease Agreement', icon: LuSignature },
    activity: { label: 'History', icon: LuHistory },
};

/** Thin horizontal scrollbar — same pattern as `LeadMainTabBar`. */
...
type Props = {
    active: ResidentWorkspaceTabId;
    onChange: (id: ResidentWorkspaceTabId) => void;
};

export function ResidentMainTabBar({ active, onChange }: Props) {
    ...
    return (
        <div className="sticky top-26 z-40 -mx-6 border-b border-gray-200 bg-white">
            <div className="relative flex min-w-0">
                ...
                {canScrollRight ? (
                    <div
                        className="pointer-events-none absolute top-0 right-0 z-10 h-full w-8 bg-linear-to-l from-white to-transparent"
                        aria-hidden
                    />
                ) : null}
                <nav
                ...
                >
                    {RESIDENT_WORKSPACE_TAB_ORDER.map((id) => {
...
export type { ResidentWorkspaceTabId };
