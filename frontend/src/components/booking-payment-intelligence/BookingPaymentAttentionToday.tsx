...
import type { BookingAttentionItem } from '@/lib/bookingPaymentIntelligenceStore';
import { BP_INTEL_SECTION_IDS, getBookingIntelHref } from '@/lib/bookingPaymentIntelligenceRoutes';
import { cn } from '@/lib/utils';

export function BookingPaymentAttentionToday({ items }: { items: BookingAttentionItem[] }) {
    if (items.length === 0) return null;

    return (
        <section
            id={BP_INTEL_SECTION_IDS.attention}
        ...
        >
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-rose-200/80 bg-rose-100/60 px-4 py-3">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-rose-800/90">Overdue installments, pending advances, and expiring payment links.</p>
                    </div>
                </div>
            ...
            <ul className="divide-y divide-rose-100/90">
                {items.map((item) => {
                    ...
                    const href = getBookingIntelHref(item.bookingSlug, 'payments');
                    const inner = (
                        <>
                            <div className="min-w-0">
                                ...
                                <p className="mt-1.5 text-sm font-bold text-slate-900">{item.customerName}</p>
                                <p className="text-xs text-slate-600">
                                    Project: <span className="font-semibold text-violet-800">{item.projectName}</span>
                                </p>
