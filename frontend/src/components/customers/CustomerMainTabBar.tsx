...
import {
    CUSTOMER_WORKSPACE_TAB_ORDER,
    type CustomerWorkspaceTabId,
} from '@/components/customers/customerDetailTabIds';
import { LuClock3, LuCreditCard, LuFileText, LuLayoutGrid, LuMegaphone } from 'react-icons/lu';
import { CTA_FOCUS_VISIBLE_RING } from '@/lib/theme/ctaThemeClasses';
import { cn } from '@/lib/utils';

const TAB_META: Record<CustomerWorkspaceTabId, { label: string; icon: typeof LuLayoutGrid }> = {
    overview: { label: 'Overview', icon: LuLayoutGrid },
    payments: { label: 'Payments', icon: LuCreditCard },
    documents: { label: 'Documents', icon: LuFileText },
    'project-updates': { label: 'Project Updates', icon: LuMegaphone },
    history: { label: 'History', icon: LuClock3 },
};
...
type Props = {
    active: CustomerWorkspaceTabId;
    onChange: (id: CustomerWorkspaceTabId) => void;
    disabledTabs?: Set<CustomerWorkspaceTabId>;
};

export function CustomerMainTabBar({ active, onChange, disabledTabs }: Props) {
    ...
    return (
        <div className="sticky top-26 z-40 -mx-6 border-b border-gray-200 bg-white">
            <div className="relative flex min-w-0">
                ...
                >
                    {CUSTOMER_WORKSPACE_TAB_ORDER.map((id) => {
