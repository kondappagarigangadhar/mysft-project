...
import {
    WORK_ORDER_DETAIL_PRIMARY_TAB_ORDER,
} from '@/components/work-orders/detail/workOrderDetailTabIds';
import { LuBell, LuHistory, LuLayoutGrid } from 'react-icons/lu';
import { cn } from '@/lib/utils';
import { CTA_RECORD_TAB_ACTIVE, CTA_RECORD_TAB_BTN_FOCUS } from '@/lib/theme/ctaThemeClasses';

const TAB_META: Record<
    WorkOrderDetailMainTabId,
...
export function WorkOrderMainTabBar({ active, onChange }: { active: WorkOrderDetailMainTabId; onChange: (id: WorkOrderDetailMainTabId) => void }) {
    ...
    const pickTab = (id: WorkOrderDetailMainTabId) => {
        onChange(id);
    };
    ...
    return (
        <div className="sticky top-26 z-40 -mx-6 border-b border-gray-200 bg-white">
            <div className="relative flex min-w-0">
                ...
                <nav ref={navRef} className={cn('flex min-w-0 flex-1 divide-x divide-gray-200 px-0', scrollNavClass)} aria-label="Work order record sections">
                    {WORK_ORDER_DETAIL_PRIMARY_TAB_ORDER.map((id) => {
