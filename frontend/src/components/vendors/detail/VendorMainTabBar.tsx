...
import type { VendorMainTabId } from '@/components/vendors/detail/vendorDetailTabIds';
import { cn } from '@/lib/utils';
import { CTA_RECORD_TAB_ACTIVE, CTA_RECORD_TAB_BTN_FOCUS } from '@/lib/theme/ctaThemeClasses';
import { LuClipboardCopy, LuHistory, LuLayoutGrid, LuTrendingUp } from 'react-icons/lu';

export type { VendorMainTabId } from '@/components/vendors/detail/vendorDetailTabIds';
...
export function VendorMainTabBar({
    active,
    onChange,
    disabledTabs,
}: {
    active: VendorMainTabId;
    onChange: (id: VendorMainTabId) => void;
    disabledTabs?: Partial<Record<VendorMainTabId, boolean>>;
}) {
    ...
    return (
        <div className="sticky top-26 z-40 -mx-6 border-b border-gray-200 bg-white">
            <div className="relative flex min-w-0">
                ...
                <nav
                    ref={navRef}
                    className="flex min-w-0 flex-1 divide-x divide-gray-200 overflow-x-auto overflow-y-hidden scroll-smooth overscroll-x-contain [scrollbar-width:thin] [scrollbar-color:theme(colors.slate.300)_theme(colors.slate.50)] [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-slate-50 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300 hover:[&::-webkit-scrollbar-thumb]:bg-slate-400"
                    aria-label="Vendor record sections"
                >
                {Object.entries(TAB_META).map(([id, meta]) => {
                    const tabId = id as VendorMainTabId;
