...
import { LuChevronDown, LuEye, LuPencil, LuPower, LuTrash2 } from 'react-icons/lu';
import { Button } from '@/components/ui/Button';
import { CTA_MENU_ITEM } from '@/lib/theme/ctaThemeClasses';
import { cn } from '@/lib/utils';
import type { Department } from '@/data/mockData';
import { departmentProfileEditHref, departmentProfileHref } from '@/lib/departmentRoutes';

const MENU_MIN_W = 220;

export function DepartmentRowActionsMenu({
    department,
    onToggleStatus,
    onDelete,
}: {
    ...
    const menuContent = (
        ...
        >
            <Link href={departmentProfileHref(department.id)} className={CTA_MENU_ITEM} onClick={() => setOpen(false)}>
                <LuEye size={16} className="text-slate-400" />
                View
            </Link>
            <Link href={departmentProfileEditHref(department.id)} className={CTA_MENU_ITEM} onClick={() => setOpen(false)}>
