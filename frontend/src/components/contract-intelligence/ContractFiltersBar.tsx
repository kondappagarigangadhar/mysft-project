...
import type { ContractRiskLevel, ContractStatus, ContractType } from '@/lib/contractIntelligenceStore';
import { CONTRACT_TYPE_OPTIONS, CONTRACT_VENDOR_OPTIONS } from '@/lib/contractIntelligenceStore';
import type { ContractIntelDatePreset, ContractIntelFilters } from '@/lib/contractIntelligenceHelpers';
...
export function ContractFiltersBar({
    ...
    filters: ContractIntelFilters;
    onChange: (patch: Partial<ContractIntelFilters>) => void;
}) {
    const presetBtn = (preset: ContractIntelDatePreset, label: string) => (
        <button
            key={preset}
            type="button"
            ...
            <div className="flex flex-col gap-3">
                ...
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                    <label className="flex flex-col gap-1">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Contract type</span>
                        <select
                            className={selectClass}
                            value={filters.contractTypeFilter}
                            onChange={(e) => onChange({ contractTypeFilter: e.target.value as 'All' | ContractType })}
                        >
                    ...
                    <label className="flex flex-col gap-1">
                        ...
                        <select
                            ...
                            onChange={(e) => onChange({ riskLevelFilter: e.target.value as 'All' | ContractRiskLevel })}
                        >
                            <option value="All">All</option>
                    ...
                    <label className="flex flex-col gap-1">
                        ...
                        <select
                            ...
                            onChange={(e) => onChange({ statusFilter: e.target.value as 'All' | ContractStatus })}
                        >
                            <option value="All">All</option>
