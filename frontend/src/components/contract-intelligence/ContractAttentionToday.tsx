...
import type { ContractAttentionItem } from '@/lib/contractIntelligenceStore';
import { getContractDetailHref } from '@/lib/contractIntelligenceRoutes';
import { CONTRACT_SECTION_IDS } from '@/lib/contractIntelligenceRoutes';
import { cn } from '@/lib/utils';

export function ContractAttentionToday({ items }: { items: ContractAttentionItem[] }) {
    if (items.length === 0) return null;

    return (
        <section
            id={CONTRACT_SECTION_IDS.attention}
        ...
        >
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-rose-200/80 bg-rose-100/60 px-4 py-3">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-rose-800/90">Critical contract, legal, and renewal issues only.</p>
                    </div>
                </div>
            ...
            <ul className="divide-y divide-rose-100/90">
                {items.map((item) => {
                    ...
                    const href = getContractDetailHref(item.contractSlug);
                    const inner = (
                        <>
                            <div className="min-w-0">
                                ...
                                <p className="mt-1.5 text-sm font-bold text-slate-900">{item.contractName}</p>
                                {item.vendor ? (
