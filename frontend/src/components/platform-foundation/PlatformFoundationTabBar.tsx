...
import { PLATFORM_FOUNDATION_TAB_ORDER } from '@/components/platform-foundation/platformFoundationTabIds';
...
const TAB_META: Record<
    PlatformFoundationTabId,
    { label: string; icon: React.ComponentType<{ className?: string; size?: number }> }
> = {
    organization: { label: 'Organization', icon: LuBuilding2 },
    security: { label: 'Security', icon: LuShield },
    subscription: { label: 'Subscription', icon: LuCreditCard },
    branding: { label: 'Branding', icon: LuPalette },
    audit: { label: 'Audit & Logs', icon: LuHistory },
};
...
export function PlatformFoundationTabBar({
    active,
    onChange,
}: {
    active: PlatformFoundationTabId;
    onChange: (id: PlatformFoundationTabId) => void;
}) {
        ...
        <div className="sticky top-26 z-40 -mx-6 border-b border-gray-200 bg-white">
            <div className="relative flex min-w-0">
                ...
                <nav
                    ...
                    aria-label="Platform foundation sections"
                >
                    {PLATFORM_FOUNDATION_TAB_ORDER.map((id) => {
