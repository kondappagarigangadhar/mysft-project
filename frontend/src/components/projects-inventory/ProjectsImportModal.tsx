...
import { bulkImportProjects } from '@/lib/projectsInventoryStore';
import {
    downloadSampleProjectsCsv,
    parseProjectsCsvContent,
    type ParsedProjectRow,
    type ProjectRowParseResult,
} from '@/lib/projectsImportCsv';
...
export function ProjectsImportModal({ isOpen, onClose, onImported }: Props) {
    const inputRef = useRef<HTMLInputElement>(null);
    const [fileName, setFileName] = useState('');
    const [preview, setPreview] = useState<ProjectRowParseResult[] | null>(null);
    ...
    const parseFile = (text: string, name: string) => {
        setFileName(name);
        setResult(null);
        setPreview(parseProjectsCsvContent(text));
    };
    ...
    const validRows: ParsedProjectRow[] =
        preview?.filter((r): r is ProjectRowParseResult & { ok: true; data: ParsedProjectRow } => r.ok === true).map((r) => r.data) ?? [];
    ...
    const handleSubmit = () => {
        ...
        setSubmitting(true);
        try {
            const res = bulkImportProjects(validRows);
            ...
            <div className="space-y-4">
                <p className="text-sm text-slate-600">
                    ...
                    <span className="font-mono text-xs">
                        project_name, project_type, location, total_units, project_status, requires_approval
                    </span>
                    . Types: <span className="font-semibold">Plot | Apartment | Villa</span>. Status:{' '}
                    <span className="font-semibold">upcoming | active | sold out</span>. Approval: <span className="font-semibold">yes | no</span>.
                </p>
                <div className="flex flex-wrap gap-2">
                    <Button type="button" variant="companyOutline" size="cta" className="gap-2" onClick={() => downloadSampleProjectsCsv()}>
                        <LuDownload size={18} />
