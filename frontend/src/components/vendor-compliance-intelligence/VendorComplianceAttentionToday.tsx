...
import type { VendorComplianceAttentionItem } from '@/lib/vendorComplianceIntelligenceStore';
import { getVendorComplianceDetailHref } from '@/lib/vendorComplianceIntelligenceRoutes';
import { VENDOR_COMPLIANCE_SECTION_IDS } from '@/lib/vendorComplianceIntelligenceRoutes';
import { cn } from '@/lib/utils';

export function VendorComplianceAttentionToday({ items }: { items: VendorComplianceAttentionItem[] }) {
    if (items.length === 0) return null;

    return (
        <section
            id={VENDOR_COMPLIANCE_SECTION_IDS.attention}
            aria-label="Compliance attention required"
        ...
        >
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-rose-200/80 bg-rose-100/60 px-4 py-3">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h2 className="text-sm font-bold text-rose-950">Compliance Attention Required</h2>
                        <p className="text-xs text-rose-800/90">Critical vendor compliance and document issues only.</p>
                    </div>
                </div>
            ...
            <ul className="divide-y divide-rose-100/90">
                {items.map((item) => {
                    ...
                    const href = getVendorComplianceDetailHref(item.vendorSlug);
                    const inner = (
                        <>
                            <div className="min-w-0">
                                ...
                                <p className="mt-1.5 text-sm font-bold text-slate-900">{item.vendorName}</p>
