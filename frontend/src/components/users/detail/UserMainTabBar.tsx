...
import { USER_DETAIL_PRIMARY_TAB_ORDER } from '@/components/users/detail/userDetailTabIds';
import { LuFileText, LuHistory, LuLayoutGrid, LuShield } from 'react-icons/lu';
import { cn } from '@/lib/utils';
import { CTA_FOCUS_VISIBLE_RING } from '@/lib/theme/ctaThemeClasses';

const TAB_META: Record<
    UserDetailMainTabId,
    { label: string; icon: React.ComponentType<{ className?: string; size?: number }>; disabled?: boolean }
> = {
    overview: { label: 'Overview', icon: LuLayoutGrid },
    roles: { label: 'Roles & Permissions', icon: LuShield },
...
export function UserMainTabBar({
    active,
    onChange,
    disabledTabs,
}: {
    active: UserDetailMainTabId;
    onChange: (id: UserDetailMainTabId) => void;
    disabledTabs?: Set<UserDetailMainTabId>;
}) {
    ...
    return (
        <div className="sticky top-26 z-40 -mx-6 border-b border-gray-200 bg-white">
            <div className="relative flex min-w-0">
                ...
                <nav ref={navRef} className={cn('flex min-w-0 flex-1 divide-x divide-gray-200 px-0', scrollNavClass)} aria-label="User record sections">
                    {USER_DETAIL_PRIMARY_TAB_ORDER.map((id) => {
