...
import { duplicateUserRecord, updateUserRecord } from '@/data/mockData';
...
export function UserDetailMoreMenu({
    user,
    onEdit,
    isEditing = false,
    isSaving = false,
    ...
    const onClone = () => {
        if (createMode || !user.id) return;
        const copy = duplicateUserRecord(user.id);
        if (copy) router.push(userViewHref(copy.id));
    };
    ...
    const onShare = async () => {
        ...
        try {
            if (navigator.share) {
                await navigator.share({ title: `${user.name} · User`, url });
                return;
            }
        } catch {
            /* ignore */
    ...
    const confirmArchive = () => {
        ...
        if (!createMode && user.id) {
            updateUserRecord(user.id, { status: 'Suspended' });
            onArchived?.();
        }
    ...
    return (
        <>
            <div className="flex flex-wrap items-center justify-start gap-3" role="toolbar" aria-label="User actions">
                {!isEditing ? (
                ...
                <button
                    type="button"
                    onClick={onClone}
                    disabled={isSaving || isEditing || createMode}
                ...
                <button
                    type="button"
                    onClick={() => router.push(userCreateHref())}
                    disabled={isSaving || isEditing}
                    className={`${actionBtn} border border-slate-200 bg-white text-slate-800 hover:bg-gray-100`}
                >
            ...
            >
                <p className="text-sm text-slate-600">
                    Archive <span className="font-semibold text-slate-900">{user.name}</span>? Status will be set to Suspended and the user will
                    leave the active directory list.
                </p>
            </Modal>
            ...
            >
                <p className="mb-2 text-sm text-slate-600">Copy this URL to share this user record.</p>
                <input
                    readOnly
                    value={shareUrl}
