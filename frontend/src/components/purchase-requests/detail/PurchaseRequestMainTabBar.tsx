...
import { PURCHASE_REQUEST_DETAIL_PRIMARY_TAB_ORDER } from '@/components/purchase-requests/detail/purchaseRequestDetailTabIds';
...
const TAB_META: Record<
    PurchaseRequestDetailMainTabId,
...
type Props = {
    active: PurchaseRequestDetailMainTabId;
    onChange: (id: PurchaseRequestDetailMainTabId) => void;
    /** When true, only Overview is enabled (e.g. create flow before first save). */
    lockedDetailTabs?: boolean;
};

export function PurchaseRequestMainTabBar({ active, onChange, lockedDetailTabs }: Props) {
    ...
    const pickTab = (id: PurchaseRequestDetailMainTabId) => {
        if (lockedDetailTabs && id !== 'overview') return;
        onChange(id);
    };
        ...
        <div className="sticky top-26 z-40 -mx-6 border-b border-gray-200 bg-white">
            <div className="relative flex min-w-0">
                ...
                <nav
                    ...
                    aria-label="Purchase request sections"
                >
                    {PURCHASE_REQUEST_DETAIL_PRIMARY_TAB_ORDER.map((id) => {
                        const item = TAB_META[id];
                        const Icon = item.icon;
                        const isActive = active === id;
                        const disabled = Boolean(lockedDetailTabs && id !== 'overview');
                        return (
                            <button
                                key={id}
                                type="button"
                                ...
                                className={cn(
                                    ...
                                    disabled ? cn('cursor-not-allowed opacity-45', CTA_RECORD_TAB_ACTIVE) : '',
                                    !disabled && isActive ? CTA_RECORD_TAB_ACTIVE : '',
                                    !disabled && !isActive ? 'font-medium text-gray-500 hover:text-gray-800' : '',
                                )}
                            >
                                <Icon
                                    size={16}
                                    className={cn('shrink-0', isActive && !disabled ? 'text-[var(--cta-button-bg)]' : 'opacity-80')}
                                    aria-hidden
                                />
                                {item.label}
                            </button>
