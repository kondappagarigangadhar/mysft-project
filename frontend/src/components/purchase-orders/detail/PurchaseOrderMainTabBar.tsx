...
import { PURCHASE_ORDER_DETAIL_PRIMARY_TAB_ORDER } from '@/components/purchase-orders/detail/purchaseOrderDetailTabIds';
...
type Props = {
    active: PurchaseOrderDetailMainTabId;
    onChange: (id: PurchaseOrderDetailMainTabId) => void;
    /** Disable non-overview tabs during PO create (matches Leads / PR). */
    secondaryDisabled?: boolean;
    historyDisabled?: boolean;
};

export function PurchaseOrderMainTabBar({ active, onChange, secondaryDisabled, historyDisabled }: Props) {
    ...
    const pickTab = (id: PurchaseOrderDetailMainTabId) => {
        if (id === 'activity' && historyDisabled) return;
        if (secondaryDisabled && id !== 'overview') return;
        onChange(id);
    };
    ...
    return (
        <div className="sticky top-26 z-40 -mx-6 border-b border-gray-200 bg-white">
            <div className="relative flex min-w-0">
                ...
                >
                    {PURCHASE_ORDER_DETAIL_PRIMARY_TAB_ORDER.map((id) => {
                        const item = TAB_META[id];
                        const Icon = item.icon;
                        const isActive = active === id;
                        const disabled = (id === 'activity' && historyDisabled) || (secondaryDisabled && id !== 'overview');
                        return (
                            <button
                                key={id}
                                type="button"
